highp vec3 transform( in highp mat4 tm, in highp vec3 v )
{
	return v * mat3(tm) + vec3(tm[3]);
}

highp vec3 rotate( in highp mat3 tm, in highp vec3 v )
{
	return v * tm;
}

// Packing a [0-1] float value into a 4D vector where each component will be a 8-bits integer
highp vec4 packFloatToVec4i(in highp float value)
{
...
highp float unpackFloatFromVec4i(in highp vec4 value)
{
